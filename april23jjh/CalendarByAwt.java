...
public class CalendarByAwt extends Frame implements ItemListener, WindowListener, ActionListener {
	...
	private EmployeeSystem es; // 시스템을 변수로선언
	...
	public CalendarByAwt(CustomerSystem cs) {
		...
		setResizable(false); // 달력 사이즈 조절할때 자유
		setVisible(true);
		chyear = new Choice();
		chmonth = new Choice();
		yLabel = new Label("년");
	...
	public CalendarByAwt(EmployeeSystem es) { // 생성자를생성후 변수로 시스템을받음
		this.es = es;
		setTitle("달력");
		setSize(550, 500);
		dimen = Toolkit.getDefaultToolkit().getScreenSize();
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		cs.textField[3].setText(year + "/" + month + "/" + day); // customer클래스에있는 textField4번째에 
